					...
					self.dom.append(Node.section([
						...
						Node.div(Node.div({className: 'detail'}, [
							...
							Node.p([
								...
								Node.span({className: 'value'}, module.packages.map((p) => Node.span({className: 'tag'}, safeHtml(p))))
							]),
							Node.p([
								Node.span({className: 'title'}, Translator.get('security.module.uses')),
								Node.span({className: 'value'}, module.uses.map((p) => Node.span({className: 'tag'}, safeHtml(p))))
							]),
							Node.p([
								Node.span({className: 'title'}, Translator.get('security.module.provides')),
								Node.span({className: 'value'}, module.provides.map((p) => Node.span({className: 'tag'}, safeHtml(p))))
							]),
							Node.p([
								Node.span({className: 'title'}, Translator.get('security.module.requires')),
								Node.span({className: 'value'}, module.requires.map((p) => Node.span({className: 'tag'}, safeHtml(p))))
							]),
							Node.p([
								Node.span({className: 'title'}, Translator.get('security.module.opens')),
								Node.span({className: 'value'}, module.opens.map((p) => Node.span({className: 'tag'}, safeHtml(p))))
							]),
							Node.p([
								Node.span({className: 'title'}, Translator.get('security.module.exports')),
								Node.span({className: 'value'}, module.exports.map((p) => Node.span({className: 'tag'}, safeHtml(p))))
							])
						]))
					]));
				}
