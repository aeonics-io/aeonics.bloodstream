...
class StatsPage extends Page
{
	async show()
	{
		...
		document.body.querySelectorAll('nav li').forEach(e => { if( e.dataset.link == 'stats') e.classList.add('selected'); else e.classList.remove('selected'); });
		...
		Ajax.get('/api/meta/usage', {data: {granularity: 'hour'}}).then((result) =>
		{
			...
			dom.querySelector('.threads').append(
				...
				.map((t, i) => Node.p([
					...
					Node.label({htmlFor: 'check_hourly_' + t.id}, safeHtml("#" + t.id + " " + t.name))
				]))
			);
		...
		Ajax.get('/api/meta/usage', {data: {granularity: 'day'}}).then((result) =>
		{
			...
			dom.querySelector('.threads').append(
				...
				.map((t, i) => Node.p([
					...
					Node.label({htmlFor: 'check_daily_' + t.id}, safeHtml("#" + t.id + " " + t.name))
				]))
			);
		...
		Ajax.get('/api/meta/usage', {data: {granularity: 'year', year: new Date().getFullYear()}}).then((result) =>
		{
			...
			dom.querySelector('.threads').append(
				...
				.map((t, i) => Node.p([
					...
					Node.label({htmlFor: 'check_yearly_' + t.id}, safeHtml("#" + t.id + " " + t.name))
				]))
			);
