...
class StoragePage extends Page
{
					...
					self.storages.map(s => Node.li({dataset: {id: s.id}}, [
						...
						Node.div(Node.div({className: 'detail'}, [
							Node.p([
								...
								Node.span({className: 'value'}, Node.a({href: '#home?entity=' + s.id}, safeHtml(s.id)))
							]),
							Node.p([
								Node.span({className: 'title'}, Translator.get('info.entity.type')),
								Node.span({className: 'value'}, safeHtml(s.type))
							]),
							Node.p([
								Node.span({className: 'title'}, Translator.get('info.entity.class')),
								Node.span({className: 'value'}, safeHtml(s.class))
							])
						]))
					]))
				)
				...
				[
					...
					self.databases.map(db => Node.li({dataset: {id: db.id}}, [
						...
						Node.div(Node.div({className: 'detail'}, [
							Node.p([
								Node.span({className: 'title'}, Translator.get('info.entity.id')),
								Node.span({className: 'value'}, Node.a({href: '#home?entity=' + db.id}, safeHtml(db.id)))
							]),
							Node.p([
								Node.span({className: 'title'}, Translator.get('info.entity.type')),
								Node.span({className: 'value'}, safeHtml(db.type))
							]),
							Node.p([
								Node.span({className: 'title'}, Translator.get('info.entity.class')),
								Node.span({className: 'value'}, safeHtml(db.class))
							])
						]))
					]))
				)
	...
	remove(id, type)
	{
		...
		var self = this;
		Modal.confirm(Translator.get('storage.remove.' + type + '.confirm', safeHtml(item.name)), [Translator.get('remove'), Translator.get('cancel')]).then((index) =>
		{
			if( index > 0 ) return;
	...
	browse(id, path)
	{
			...
			[
				...
				Node.ol({tabIndex: -1,
					...
					keydown: function(e)
					{
						...
						{
							Modal.confirm(Translator.get('storage.remove.item.confirm', safeHtml(s.dataset.path)), [Translator.get('remove'), Translator.get('cancel')]).then((index) =>
							{
								if( index > 0 ) return;
								div.classList.add('wait');
		...
		Ajax.get('/api/admin/storage/' + encodeURIComponent(id), {data: {path: path}}).then((result) =>
		{
			...
			{
				ol.append(Node.li({className: 'folder', title: p, dataset: {path: path + p}}, safeHtml(p.slice(0, -1))));
			});

			result.response.filter(p => !p.endsWith('/')).forEach(p =>
			{
				ol.append(Node.li({className: 'file', title: p, dataset: {path: path + p}}, safeHtml(p)));
			});
					...
					[
						Node.div({className: 'group'}, [
							Node.p([
								...
								Node.span({className: 'text'}, safeHtml(""+result.status))
							]),
							Node.p([
