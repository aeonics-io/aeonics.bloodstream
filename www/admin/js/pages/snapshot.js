		...
		]).then((results) =>
		{
			...
			section.append(
				...
				Node.div(Node.div({className: 'detail'}, [
					Node.p([
						...
						Node.span({className: 'value'}, Node.a({href: '#home?entity=' + current.id}, safeHtml(current.id)))
					]),
					Node.p([
						Node.span({className: 'title'}, Translator.get('info.template.type_plugin')),
						Node.span({className: 'value'}, safeHtml(current.plugin))
					]),
					Node.p([
						Node.span({className: 'title'}, Translator.get('info.entity.type')),
						Node.span({className: 'value'}, safeHtml(current.type))
					]),
					Node.p([
						Node.span({className: 'title'}, Translator.get('info.entity.class')),
						Node.span({className: 'value'}, safeHtml(current.class))
					]),
					Node.p([
						Node.span({className: 'title'}, Translator.get('snapshot.latest')),
						Node.span({className: 'text'}, safeHtml(latest))
					])
				]))
			);
			...
			else
			{
				list.forEach((snapshot) =>
				{
					...
					div.append(Node.li([
						Node.h3(safeHtml(name||snapshot)),
						Node.div(Node.div({className: 'detail'}, [
							Node.p([
								Node.span({className: 'title'}, Translator.get('snapshot.info.name')),
								Node.span({className: 'text'}, safeHtml(snapshot))
							]),
							Node.p([
