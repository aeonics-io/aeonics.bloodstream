import { Page, Node, Ajax, Translator, Notify } from 'core';
import { css, safeHtml, urlValue, config } from 'core';
...
await loadScript(config.sitePath + 'js/ext/d3-quadtree.min.js');
await loadScript(config.sitePath + 'js/ext/d3-force.min.js');
await loadScript(config.sitePath + 'js/ext/force-graph.min.js');
...
class HomePage extends Page
{
	async show()
	{
		...
		document.body.querySelectorAll('nav li').forEach(e => { if( e.dataset.link == 'home') e.classList.add('selected'); else e.classList.remove('selected'); });
	...
	showInfo(node)
	{
		...
		Ajax.get('/api/meta/entity/' + node.category + '/' + node.id).then((response) =>
		{
			...
			Ajax.get('/api/meta/template/' + node.category + '/' + node.subtype).then((response) =>
			{
				...
				i.append(
					Node.h1(safeHtml(entity.name||'-no name-')),
		...
		return Node.section({className: 'open'}, [
			...
			Node.div(Node.div({className: 'detail'}, [
				Node.p([
					...
					Node.span({className: 'value'}, safeHtml(entity.id)),
				...
				Node.p([
					...
					Node.span({className: 'value'}, safeHtml(entity.category)),
				...
				Node.p([
					...
					Node.span({className: 'value'}, safeHtml(entity.type)),
					Node.span({className: 'icon ref',
				...
				Node.p([
					...
					Node.span({className: 'value'}, safeHtml(entity.class))
				]),
				Node.p([
		...
		return Node.section({className: 'open'}, [
			...
			Node.div(Node.div({className: 'detail'}, [
				Node.p([
					...
					Node.span({className: 'text'}, safeHtml(template.summary))
				]),
				Node.p([
					Node.span({className: 'title'}, Translator.get('info.template.description')),
					Node.span({className: 'text'}, safeHtml(template.description))
				]),
				Node.p([
					Node.span({className: 'title'}, Translator.get('info.template.category')),
					Node.span({className: 'value'}, safeHtml(template.category)),
					Node.span({className: 'icon ref',
				...
				Node.p([
					...
					Node.span({className: 'value'}, safeHtml(template.type))
				]),
				Node.p([
					Node.span({className: 'title'}, Translator.get('info.template.type_plugin')),
					Node.span({className: 'value'}, safeHtml(template.type_plugin))
				]),
				Node.p([
					Node.span({className: 'title'}, Translator.get('info.template.target')),
					Node.span({className: 'value'}, safeHtml(template.target))
				]),
				Node.p([
					Node.span({className: 'title'}, Translator.get('info.template.target_plugin')),
					Node.span({className: 'value'}, safeHtml(template.target_plugin))
				]),
				Node.p([
					Node.span({className: 'title'}, Translator.get('info.template.class')),
					Node.span({className: 'value'}, safeHtml(template.class))
				]),
				Node.p([
		...
		return Node.section({className: 'open'}, [
			...
			Node.div(Node.div({className: 'detail'}, Object.values(inputs).concat(Object.values(outputs))
				...
				.map((c) => Node.div({className: 'sublevel'}, [
					Node.h3(safeHtml(c.name)),
					Node.p([
						Node.span({className: 'title'}, Translator.get('info.template.summary')),
						Node.span({className: 'text'}, safeHtml(c.summary))
					]),
					Node.p([
						Node.span({className: 'title'}, Translator.get('info.template.description')),
						Node.span({className: 'text'}, safeHtml(c.description))
					]),
					Node.p([
	...
	getParameterInfo(param, entity, config)
	{
		return Node.div([
			Node.fieldset({dataset: {config: config}}, [
				Node.label({htmlFor: '__' + param.name}, safeHtml(param.name)),
			...
			Node.div({className: 'group'}, [
				Node.p([
					...
					Node.span({className: 'text'}, safeHtml(param.summary)),
				]),
				Node.p([
					Node.span({className: 'title'}, Translator.get('info.config.description')),
					Node.span({className: 'text'}, safeHtml(param.description)),
				]),
				Node.p([
					Node.span({className: 'title'}, Translator.get('info.config.config')),
					Node.span({className: 'value'}, !!config ? Translator.get('info.config.global', safeHtml(config)) : Translator.get('info.config.local')),
				]),
				Node.p([
					Node.span({className: 'title'}, Translator.get('info.config.format')),
					Node.span({className: 'value'}, safeHtml(param.format)),
				]),
				Node.p([
				...
				Node.p([
					...
					Node.span({className: 'value'}, safeHtml(param.min + " / " + param.max)),
				]),
				Node.p([
					Node.span({className: 'title'}, Translator.get('info.config.default')),
					Node.span({className: 'value'}, safeHtml(JSON.stringify(param.defaultValue))),
				]),
				Node.p([
				...
				Node.p([
					...
					Node.span({className: 'value'}, param.values.length > 0 ? safeHtml(JSON.stringify(param.values)) : ''),
				]),
				Node.p([
	...
	getRelationInfo(entity, template)
	{
		var self = this;
		...
		var content = this.data.links
			...
			.map((l) => Node.p([
				...
				Node.span({className: 'value'}, Translator.get('info.link.target2', safeHtml(l.source.name), safeHtml(l.source.subtype))),
				Node.span({className: 'icon ref',
	...
	getRelationDetail(rel, ref)
	{
		...
		ref.forEach((r) =>
		{
			...
			refs.push(Node.p([
				Node.span({className: 'value'}, Translator.get('info.link.target2', safeHtml(e.name), safeHtml(e.subtype))),
				/*
		...
		*/
		...
		return Node.p([
			Node.span({className: 'title orange'}, safeHtml(rel.name)),
	...
	listRegistry(category)
	{
		...
		i.append(
			Node.h1(safeHtml(category||'-no name-')),
			...
			Node.ul(this.data.nodes
				...
				.map((n) => Node.li({dataset: {id: n.id}, click: function() { self.nodeClick(this.dataset.id); }}, safeHtml(n.name)))
	...
	listFactory(category)
	{
		...
		var sections = types.sort().map((t) => Node.section({className: 'open'}, [
			Node.h2({click: function() { this.parentNode.classList.toggle('open'); }}, safeHtml(t)),
	...
	listPlugin(name)
	{
		...
		var lis = this.data.links
			...
			.map((l) => Node.li({dataset: {id: l.target.id}, click: function() { self.nodeClick(this.dataset.id); }}, safeHtml(l.target.name)))
			.sort((a, b) => a.textContent.toLowerCase().localeCompare(b.textContent.toLowerCase()));

		i.append(
			Node.h1(safeHtml(name||'-no name-')),
			Node.div({className: 'search'}, [
				Node.input({type: 'search', input: function()
				{
					var value = this.value;
	...
	getData()
	{
		...
		Ajax.get('/api/meta/overview').then((response) =>
		{
			...
			// highlight node in query string
			var id = urlValue('entity');
			if( id ) setTimeout(function() { self.nodeClick(id); }, 300);
		}, (error) =>
		{
			Notify.error(Translator.get('fetch.error'));
		});
	...
	fixPosition(quadrant, main, nodes)
	{
