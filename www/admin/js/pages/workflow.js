...
var x = new Promise((ok, nok) =>
{
	ae.require('Page', 'Node', 'Ajax', 'Translator', 'Notify', 'Modal', 'Entity', 'page.workflow.css').then(([Page, Node, Ajax, Translator, Notify, Modal, Entity]) =>
	{
		...
		Object.assign(page, 
		{
			...
			refreshList: function()
			{
				...
				Ajax.get('/api/meta/registry/aeonics.entity.flow/entities').then((result) =>
				{
					...
					list.append(Node.li({dataset: {id: "10000000-ffffffffffffffff"}}, 
						[
							Node.h2([
								Node.span({click: function() { location.href = '#workflow?flow='+this.closest('li').dataset.id; }}, ae.safeHtml(Translator.get("wokflow.orphan.title"))),
								Node.div({className: 'actions'},
								[
			...
			{
				this.hide();
				this.initFlow(ae.urlValue('flow'));
			},
			...
			infoLink2: function(from, from_template, to, to_template, relation, data)
			{
				var self = this;
				...
				[
					...
					[
						...
						[
							...
							[
								Node.div([
									Node.span({className: 'itemicon'}, ae.safeHtml(from_template.icon)),
									Node.p(ae.safeHtml(from.name))
								]),
								Node.span({className: 'channel'}, ae.safeHtml(from_channel.summary))
							]),
							Node.td({className: 'has_in'}),
							Node.td(
							[
								Node.span({className: 'channel'}, ae.safeHtml(to_channel.summary)),
								Node.div([
									Node.span({className: 'itemicon'}, ae.safeHtml(to_template.icon)),
									Node.p(ae.safeHtml(to.name))
								])
							])
						]))
					]),
					...
					[
						...
						[
							...
							Node.span({className: 'itemicon'}, ae.safeHtml(to_template.icon)),
							Node.span(ae.safeHtml(to.name))
						]),
						Node.div(Node.div({className: 'detail'}, [
							Node.p([
								Node.span({className: 'title'}, Translator.get('workflow.entity.id')),
								Node.span({className: 'value'}, Node.a({href: '#home?entity=' + to.id}, ae.safeHtml(to.id)))
							]),
							Node.p([
								Node.span({className: 'title'}, Translator.get('workflow.entity.summary')),
								Node.span({className: 'value'}, ae.safeHtml(to_template.summary))
							]),
							Node.p([
								Node.span({className: 'title'}, Translator.get('workflow.entity.description')),
								Node.span({className: 'value'}, ae.safeHtml(to_template.description))
							]),
							Node.p([
								Node.span({className: 'title'}, Translator.get('workflow.entity.channel.in')),
								Node.span({className: 'text'}, ae.safeHtml(to_channel.description))
							])
						]))
					]),
					...
					[
						...
						Node.div(Node.div({className: 'properties'}, 
							properties.map(([name, detail]) =>
							{
								...
								[
									Node.label({htmlFor: 'entity__' + name}, ae.safeHtml(detail.summary)),
					...
					{
						x.preventDefault();
						Modal.confirm(Translator.get('workflow.link.remove.confirm', ae.safeHtml(from.name), ae.safeHtml(to.name)), [Translator.get('remove'), Translator.get('cancel')]).then((index) =>
						{
							if( index > 0 ) { return; }
					...
					content.append(
						...
						[
							...
							[
								Node.td(
									...
									Node.span({className: 'channel'}, ae.safeHtml(detail.summary)),
									Node.br()
									])),
								Node.td({className: ((Object.entries(t.inputs||{}).length > 0) ? 'has_in' : '') + ((Object.entries(t.outputs||{}).length > 0) ? ' has_out' : '')},
									Node.span({className: 'itemicon'}, ae.safeHtml(t.icon))),
						...
						[
							...
							Node.div(Node.div({className: 'detail'}, [
								Node.p([
									Node.span({className: 'title'}, Translator.get('workflow.entity.id')),
									Node.span({className: 'value'}, Node.a({href: '#home?entity=' + e.id}, ae.safeHtml(e.id)))
								]),
								Node.p([
									Node.span({className: 'title'}, Translator.get('workflow.entity.summary')),
									Node.span({className: 'value'}, ae.safeHtml(t.summary))
								]),
								Node.p([
									Node.span({className: 'title'}, Translator.get('workflow.entity.description')),
									Node.span({className: 'text'}, ae.safeHtml(t.description))
								])
							]))
						]),
								...
								Object.entries(t.inputs||{}).map(([name, detail]) =>
								{
									return [
										...
										Node.p([
											...
											Node.span({className: 'text'}, ae.safeHtml(detail.description))
										]),
										Node.hr()
...
// TODO : if unmapped : move to other flow
			...
			addLink: function(step_from, channel_from, step_to, channel_to)
			{
						...
						Node.div({click: function(e)
						{
							...
							self.chooseLink(1).then(result =>
							{
								...
								this.append(Node.div([
									Node.span({className: 'itemicon'}, ae.safeHtml(e.icon)),
									Node.p(ae.safeHtml(e.name)),
									Node.span({className: 'channel'}, ae.safeHtml(c.summary)),
...
// TODO : link parameters
						...
						[
							...
							[
								Node.span({className: 'itemicon'}, ae.safeHtml(step_from.icon)),
								Node.p(ae.safeHtml(step_from.name)),
								Node.span({className: 'channel'}, ae.safeHtml(channel_from.summary)),
						...
						[
							...
							[
								Node.span({className: 'itemicon'}, ae.safeHtml(step_to.icon)),
								Node.p(ae.safeHtml(step_to.name)),
								Node.span({className: 'channel'}, ae.safeHtml(channel_to.summary)),
			...
			getChannelProperties: function(step, channel)
			{
				
			},
			
			chooseLink: function(from)
			{
				var self = this;
				var ok, nok;
						...
						[
							...
							Node.div(Node.ul({className: 'stepList'}, steps.map(step => step.role != role ? null :
								...
								[
									Node.h3(
									[
										Node.span({className: 'itemicon'}, ae.safeHtml(step.icon)),
										Node.span(ae.safeHtml(step.summary))
									]),
									Node.p(ae.safeHtml(step.description))
								])
							)))
						]));
					});
			...
			addEntity2: function(template)
			{
				...
				[
					Node.h2(ae.safeHtml(template.summary)),
					Node.form(
					[
