...
class LogsPage extends Page
{
	async show()
	{
		...
		document.body.querySelectorAll('nav li').forEach(e => { if( e.dataset.link == 'logs') e.classList.add('selected'); else e.classList.remove('selected'); });
	...
	dataHandler(data)
	{
		...
		li.append(
			...
			Node.span({className: 'tag'}, safeHtml(data.type)),
			Node.p({className: 'message'}, safeHtml(data.message)),
		);
