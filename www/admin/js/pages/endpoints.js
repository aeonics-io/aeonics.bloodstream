...
class EndpointsPage extends Page
{
	async show()
	{
		...
		document.body.querySelectorAll('nav li').forEach(e => { if( e.dataset.link == 'endpoints') e.classList.add('selected'); else e.classList.remove('selected'); });
			...
			self.endpoints.forEach((r) =>
			{
				...
				if( category != (leading + '/' + type) )
				{
					...
					div.appendChild(Node.section({className: 'open'}, [
						Node.h2({click: function() { this.parentNode.classList.toggle('open'); }}, safeHtml(leading + '/' + type)),
						Node.div(ul)
					]));
				}
				...
				[
					Node.div({className: 'header', click: function(e)
					{
						...
						Node.span({className:'rest_method ' + r.method}, safeHtml(r.method)),
						Node.span({className: 'rest_path'}, safeHtml(r.url))
					]),
					Node.div({className: 'detail'}, [
						Node.p([
							Node.span({className: 'title'}, Translator.get('endpoints.summary')),
							Node.span({className: template.summary ? 'text' : 'value'}, safeHtml(template.summary||Translator.get('endpoints.no_summary')))
						]),
						Node.p([
							Node.span({className: 'title'}, Translator.get('endpoints.description')),
							Node.span({className: template.description ? 'text' : 'value'}, safeHtml(template.description||Translator.get('endpoints.no_description')))
						]),
						Node.p([
							...
							Node.span({className: 'value'}, Object.keys(template.parameters).length > 0 ? Object.keys(template.parameters).map(p =>
								Node.span({className: 'tag', dataset: {type: template.type, name: p}, click: function() { self.parameterInfo(this.dataset.type, this.dataset.name); }}, safeHtml(p)))
								: Translator.get('endpoints.no_parameters')
							)
						]),
						Node.p([
							Node.span({className: 'title'}, Translator.get('endpoints.returns')),
							Node.span({className: template.returns ? 'text' : 'value'}, safeHtml(template.returns||Translator.get('endpoints.no_returns')))
						]),
						Node.p([
	...
	testEndpoint(id)
	{
		...
		[
			...
			[
				Node.div(
				[
					Node.span({className: 'rest_method ' + e.method}, safeHtml(e.method)),
					Node.span({className: 'rest_path ' + e.url}, safeHtml(e.url))
				]),
						...
						[
							Node.div({className: 'group'}, [
								Node.p([
									...
									Node.span({className: 'text'}, safeHtml(""+response.status))
								]),
								Node.p([
				...
				}, (error) =>
				{
					...
					[
						Node.div({className: 'group'}, [
							Node.p([
								Node.span({className: 'title'}, Translator.get('endpoints.result.status')),
								Node.span({className: 'text'}, safeHtml(""+error.status))
							]),
							Node.p([
								Node.span({className: 'title'}, Translator.get('endpoints.result.roundtrip')),
								Node.span({className: 'value'}, roundtrip + "ms")
	...
	wizard()
	{
		...
		Modal.prompt(Translator.get('endpoints.wizard.title'), Node.form([
			Node.select({name: 'method'}, [
				...
				Node.option({value: 'POST'}, 'POST'),
				Node.option({value: 'PUT'}, 'PUT'),
				Node.option({value: 'DELETE'}, 'DELETE')
			]),
			...
			Node.button({click: function(e) {
				...
				}, (error) =>
				{
					...
					if( error.response.error && error.response.error.length > 0 )
					{
						...
						Modal.alert(Translator.get('endpoints.wizard.fail2', safeHtml(""+error[1]), safeHtml(""+error[3])));
					}
				});
