...
{
	ae.require('Page', 'Node', 'Ajax', 'Translator', 'Notify', 'Modal', 'page.metrics.css').then(([Page, Node, Ajax, Translator, Notify, Modal]) =>
	{
		var page = new Page();
		Object.assign(page, 
		{
							...
							Object.entries(l3).forEach(([metric, l4]) =>
							{
								...
								if( !tr )
								{
									tr = Node.tr({dataset: {uuid: uuid}}, [
										Node.td(ae.safeHtml(category)),
										Node.td(ae.safeHtml(type)),
										Node.td(ae.safeHtml(id)),
										Node.td({className: 'centered'}, ae.safeHtml(metric))
									]);
			...
			getCountByTime: function(count, seconds)
			{	
			...
			countdown: function()
			{
				...
				var left = Math.round(Math.max(0, (this.timeWindow - (new Date().getTime() - this.__until)) / 1000));
